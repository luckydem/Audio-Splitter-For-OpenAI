...
from split_audio import split_audio, split_audio_parallel, get_audio_info, get_optimal_output_format, calculate_chunk_duration
    ...
    try:
        ...
        if not needs_splitting(temp_input_path, file_size_bytes, file_name):
            ...
            with tempfile.TemporaryDirectory() as chunk_temp_dir:
                ...
                loop = asyncio.get_event_loop()
                # Use parallel processing for WAV format
                if output_format == 'wav':
                    logger.info(f"Job {job_id}: Using PARALLEL processing with 6 workers")
                    created_files = await loop.run_in_executor(
                        executor,
                        split_audio_parallel,
                        temp_input_path,
                        chunk_duration,
                        output_dir,
                        output_format,
